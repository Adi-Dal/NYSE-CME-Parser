...
class nyse_parser{
    private:
        ...
        queue<vector<message>> tasks;
        mutex m;
        condition_variable cv;
        queue<vector<message>> prlTasks;
        mutex m2;
        condition_variable cv2;
        queue<vector<message>> trdTasks;
        mutex m3;
        ...
        // addOrderMessage
        void parse100(stringstream& content, const message& message100){
        ...
        void parse100Prl(stringstream& prl_content, const message& message100){
            uint32_t sourceTimeNS = static_cast<uint32_t>(message100.buffer[7] << 24) | static_cast<uint32_t>(message100.buffer[6] << 16) | static_cast<uint32_t>(message100.buffer[5] << 8) | message100.buffer[4];
            uint32_t symbolIDX = static_cast<uint32_t>(message100.buffer[11] << 24) | static_cast<uint32_t>(message100.buffer[10] << 16) | static_cast<uint32_t>(message100.buffer[9] << 8) | message100.buffer[8];
            uint32_t symbolSeqNum = static_cast<uint32_t>(message100.buffer[15] << 24) | static_cast<uint32_t>(message100.buffer[14] << 16) | static_cast<uint32_t>(message100.buffer[13] << 8) | message100.buffer[12];
            uint64_t orderID = (static_cast<uint64_t>(message100.buffer[23]) << 56) |(static_cast<uint64_t>(message100.buffer[22]) << 48) |(static_cast<uint64_t>(message100.buffer[21]) << 40) |(static_cast<uint64_t>(message100.buffer[20]) << 32) |static_cast<uint64_t>(message100.buffer[19] << 24) | static_cast<uint64_t>(message100.buffer[18] << 16) | static_cast<uint64_t>(message100.buffer[17] << 8) | message100.buffer[16];
        ...
        // modifyOrderMessage
        void parse101(stringstream& content, const message& message101){
        ...
        // deleteOrderMessage
        void parse102(stringstream& content, const message& message102){
        ...
        void parse103(stringstream& content, const message& message103){
        ...
        // replaceOrderMessage
        void parse104(stringstream& content, const message& message104){
        ...
        void parse106(stringstream& content, const message& message100){
        ...
        void parse110(stringstream& content, const message& message110){
    ...
    public:
        ...
        void writeToFile(const vector<message>& messList){
            ...
            for(const message& tempM: messList) {
                ...
                if(tempM.msgType==100) parse100(content,tempM);
                if(tempM.msgType==101) parse101(content,tempM);
                if(tempM.msgType==102) parse102(content,tempM);
                if(tempM.msgType==103) parse103(content,tempM);
                if(tempM.msgType==104) parse104(content,tempM);
                if(tempM.msgType==106) parse106(content,tempM);
                if(tempM.msgType==110) parse110(content,tempM);
                content << "\n";
            }
            outputFile << content.str();
        }
        void writeToFilePRL(const vector<message>& messList){
            stringstream prlContent;

            for(const message& tempM: messList) {
                if(tempM.msgType==100) parse100Prl(prlContent,tempM);
            }
        ...
        void threadWrite(){
            while(true){
                ...
                vector<message> task = tasks.front();
                tasks.pop();
                lock.unlock();
        ...
        void threadWritePRL(){
            while(true){
                ...
                while(prlTasks.size()==0){
                    cv2.wait(lock);
                }
                // cout << tasks.size() << endl;
                vector<message> task = prlTasks.front();
                prlTasks.pop();
                lock.unlock();
                // cout << "In thread func" << endl;
                if(task.size()==0) break;
        ...
        void parse() {
            ...
            prlOutput.open(prl_output_file);
            trdOutput.open(trd_output_file);

            if(!outputFile.is_open()){ 
                cout << "Could not open file " << output_file <<"." << endl;
            }
            if(!prlOutput.is_open()){
                cout << "Could not open file " << prl_output_file <<"." << endl;
            }
            if(!trdOutput.is_open()){
                cout << "Could not open file " << trd_output_file <<"." << endl;
            }

            // Write CSV Headers
            outputFile << "PacketCaptureTime,SendTime,SendTimeNS,MsgSize,MsgType,SourceTime,SourceTimeNS,SymbolIndex,SymbolSeqNum,OrderID,Price,Volume,Side,Firm,PositionChange,PrintFlag,Trd1,Trd2,Trd3,Trd4,TradeID\n";
            prlOutput << "PacketCaptureTime,SendTime,Price,Side,Symbol,Size,RecordType,OrderID,NumParticipants,EventFlag,TickType\n";
             ...
             */
            trdOutput << "PacketCaptureTime,SendTime,MessageID,RawTimestamp,TickType,Symbol,Price,Size,RecordType,Flag,Ask\n";

            int byteNum = bytes.size();
            int packetNum = 0;
            // int batchNum = 0;
            thread general = thread(&nyse_parser::threadWrite,this);
            unique_lock<mutex> lock(m,defer_lock);
            thread prlHandler = thread(&nyse_parser::threadWritePRL,this);
            ...
            while(byteNum > 0){
                ...
                if(messageList.size() > batch_size){
                    ...
                    //main thread
                    lock.lock();
                    tasks.push(messageList);
                    cv.notify_one();
                    lock.unlock(); 
                    //prl thread
                    lock2.lock();
                    prlTasks.push(messageList);
                    cv2.notify_one();
                    lock2.unlock(); 
            ...
            tasks.push(move(empty));
            cv.notify_one();
            lock.unlock();
            
            lock2.lock();
            prlTasks.push(move(empty));
            cv2.notify_one();
            lock2.unlock();
...
//     string out = "orderbookThread.csv";
//     string out_prl = "NYSE_PRL_output_Thread.csv";
//     string out_trd = "NYSE_TRD_output_Thread.csv";
